...
int start() {
   ...
   // TDL 2: Assign Values to variables
     ...
     ma_faster = NormalizeDouble(iMA(NULL, 0, FastMA_period, 0, MODE_EMA, PRICE_CLOSE, 1), Digits);
     ma_slower = NormalizeDouble(iMA(NULL, 0, SlowMA_period, 0, MODE_EMA, PRICE_CLOSE, 1), Digits);
     
     macd_main_prev = NormalizeDouble(iMACD(NULL, 0, 12, 26, 9, PRICE_CLOSE, MODE_MAIN, 2), Digits);
     macd_main_actual = NormalizeDouble(iMACD(NULL, 0, 12, 26, 9, PRICE_CLOSE, MODE_MAIN, 1), Digits);
     macd_signal_prev = NormalizeDouble(iMACD(NULL, 0, 12, 26, 9, PRICE_CLOSE, MODE_SIGNAL, 2), Digits);
     macd_signal_actual = NormalizeDouble(iMACD(NULL, 0, 12, 26, 9, PRICE_CLOSE, MODE_SIGNAL, 1), Digits);
   ...
   */
   ...
   //Buy
   if (Order == SIGNAL_BUY) {
      Print(Symbol(), Period(), " BUY Signal: MACD main actual: ",DoubleToString(macd_main_actual,Digits),", MACD signal actual: ",DoubleToString(macd_signal_actual,Digits),", Close: ", Close[1]);
      if(!IsTrade) {
         //Check free margin
   ...
   if (Order == SIGNAL_SELL) {
      Print(Symbol(), Period(), " SELL Signal: MACD main actual: ",DoubleToString(macd_main_actual,Digits),", MACD signal actual: ",DoubleToString(macd_signal_actual,Digits),", Close: ", Close[1]);
      if(!IsTrade) {
         //Check free margin
         if (AccountFreeMargin() < (1000 * Lots)) {
            Print("We have no money. Free Margin = ", AccountFreeMargin());
